...
const filterCondition = itemsObject.filter(({ quantity, price }) => {
  return quantity > 2 && price > 300;
});
console.log(filterCondition);

const total = itemsObject.reduce((acc, { quantity, price }) => {
...
const expectedString = string.split(/[- ]+/).join(" ").toLowerCase().trim();
...
function merge(first, second) {
  let arrayMerge = {};
  let expectedArray = [...first, ...second];

  expectedArray.forEach((ele) => {
    arrayMerge[ele.uuid] = {
  ...
  expectedArray = Object.values(arrayMerge).sort((a, b) => a.uuid - b.uuid);
